...
public class ColumnBasedORCReceiver extends SpeedupReceiver {

private static final Logger logger = LogManager.getLogger(NaiveORCReceiver.class);
	
	private long totalBytesReceived = 0;
	private ColumnBasedORCChunkStore chunkStore = new ColumnBasedORCChunkStore();
	...
	private TransferStats handleSpecialChunk(String fileName, InputStream is, OutputStream os, FileOutputStream fos) throws IOException {
		...
		// do we have it?
		ColumnBasedORCFileChunk chunk = chunkStore.findChunkBySignature(hash);
		if(chunk != null) {
		...
		// nope, we dont, we need to send a request back
			...
			// and save it...
			try {
				List<ColumnBasedORCFileChunk> chunks = chunkStore.addChunks(content, algorithm);
	...
	private ChunkType readNextType(InputStream is) throws IOException {
		...
		return ColumnBasedORCFileChunk.fromOrdinal(BytesUtil.bytesToInt(nextTypeOrdinal));
	}
